...
. Address Book Access
* The application begins by authenticating with the CardDAV server using configured credentials.
* It fetches all contacts from the specified address book URL and processes each contact to extract relevant information, such as names and birthdays.
...
* The application interacts with the configured CalDAV server and clears all pre-existing events in the specified birthday calendar that match the defined category (_Brute-Force-Approach_). This ensures no duplicate entries are present.
* Individual birthday events are then uploaded to the remote calendar, where they are stored as `.ics` files.
