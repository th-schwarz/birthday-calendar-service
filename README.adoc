...
= Birthday Calendar Generator :: BCG

image:https://github.com/th-schwarz/BirthdayCalendarGenerator/actions/workflows/maven.yml/badge.svg["GitHub CI Build Status",link="https://github.com/th-schwarz/BirthdayCalendarGenerator/actions/workflows/maven.yml"]
image:https://img.shields.io/github/license/th-schwarz/BirthdayCalendarGenerator["License badge for BirthdayCalendarGenerator",link="https://github.com/th-schwarz/BirthdayCalendarGenerator/blob/develop/LICENSE"]

Give it a try!
https://github.com/th-schwarz/BirthdayCalendarGenerator/releases[image:https://img.shields.io/github/v/release/th-schwarz/BirthdayCalendarGenerator?include_prereleases[GitHub
release (latest by date including pre-releases)]]         image:https://img.shields.io/github/downloads/th-schwarz/BirthdayCalendarGenerator/total[GitHub Downloads (all assets, all releases)]
image:https://ghcr-badge.elias.eu.org/shield/th-schwarz/BirthdayCalendarGenerator/bcg[] +
...
BCG (Birthday Calendar Generator) is a Java Spring Boot application for creating (and syncing) a birthday calendar from address book data. It's based on spring boot 3, that’s why Java 17 or later is required! It  can be executed by `commandline`, `init.d`, `systemd`, or run in a container via `Docker`.

If you find a bug or certain features are missing, don’t hesitate to file an issue on https://github.com/th-schwarz/BirthdayCalendarGenerator/issues[Github].
...
All backend tests: image:https://github.com/th-schwarz/BirthdayCalendarGenerator/actions/workflows/backend-tests.yml/badge.svg["GitHub CI All Backend Tests",link="https://github.com/th-schwarz/BirthdayCalendarGenerator/actions/workflows/backend-tests.yml"]

Single backend tests:

* image:https://github.com/th-schwarz/BirthdayCalendarGenerator/actions/workflows/backend-baikal.yml/badge.svg["GitHub CI Baikal Backend Tests",link="https://github.com/th-schwarz/BirthdayCalendarGenerator/actions/workflows/backend-baikal.yml"]
* image:https://github.com/th-schwarz/BirthdayCalendarGenerator/actions/workflows/backend-sogo.yml/badge.svg["GitHub CI Sogo Backend Tests",link="https://github.com/th-schwarz/BirthdayCalendarGenerator/actions/workflows/backend-sogo.yml"]

== Changelog

* v3.0.0 +
Fixed an issue with reading birthday events for some dav backends e.g. Baikal.
** issue #14 improve sync
* v2.0.2
** issue #10 Docker support: any plan to release an image or accept PRs for a Dockerfile + CI?
** issue #12 The 'method' property for a calendar object causes an exception in Baikal
* v2.0.1
** issue #8 bcg terminates if the connection to the dav server fails
** issue #9 default of bcg.run-on-start should be false
* v2.0.0 +
  issue #4 implement a sync +
  instead of the Brute-Force-Approach
* v1.2.0 +
  issue #3 reminder for all birthday events
* v1.1.0 +
  issue #2: A remote event should only be deleted if its categories contain the defined birthday category.

== Basic Workflow

The Birthday Calendar Generator (BCG) is an application designed to generate a birthday calendar from the contact information stored in a CardDAV address book and upload it to a CalDAV calendar. The working process is built upon key parts and is automated using scheduling methods.
...
- `bcg.run-on-start`: Optional parameter, default is false. If it true, the sync starts immediately on start.
- `bcg.cron`: The cron trigger used to schedule the synchronization of birthdays.
...
- `dav.cal-url`: The url of the birthday calendar which must be created. _To be on the safe side, it's a good idea to use an extra birthday calendar! But it is not mandatory since https://github.com/th-schwarz/BirthdayCalendarGenerator/issues/2[A remote event should only be deleted if its categories contain the defined birthday category.]_ was implemented.
- `dav.delay-in-minutes`: This property determines the delay interval (in minutes) between retries in the case of network problems. The default setting is _1_.
- `dav.retry-delay-in-minutes`: The maximum number of retries due to network errors. The default setting is _5_.
...
BCG can be run directly from a container image hosted on GitHub Container Registry (GHCR).

[source,bash]
----
# Run with configuration file mounted from the host
docker run --rm \
  -v /path/to/bcg.yml:/app/bcg.yml:ro \
  -v /path/to/logback.xml:/app/logback.xml:ro \
  ghcr.io/th-schwarz/bcg:[tagged-version]
----

Replace `/path/to/bcg.yml` and `/path/to/logback.xml` with the absolute paths to your configuration and logging files on the host system.
...
docker run --rm \
  -v /path/to/bcg.yml:/app/bcg.yml:ro \
  -v /path/to/logback.xml:/app/logback.xml:ro \
  ghcr.io/th-schwarz/bcg:latest --run-once
----

=== by Docker Compose

You can also run BCG with Docker Compose for easier service management.

[source,yaml]
----
services:
  bcg:
    image: ghcr.io/th-schwarz/bcg:latest   # replace with ghcr.io/th-schwarz/bcg:<tag-name> if needed
    container_name: bcg
    restart: unless-stopped
    volumes:
      - ./bcg.yml:/app/bcg.yml:ro          # required configuration
...
Download the https://github.com/th-schwarz/BirthdayCalendarGenerator/releases[last release jar].
...
----
cd /opt/bcg/
java -jar bcg.jar --logging.config=logback.xml --run-once
----
