...
:sonarURL: https://sonarcloud.io/dashboard?id=th-schwarz_birthday-calendar-service
:sonarSummaryURL: https://sonarcloud.io/summary/new_code?id=th-schwarz_birthday-calendar-service
:sonarBadgeURL: https://sonarcloud.io/api/project_badges/measure?project=th-schwarz_birthday-calendar-service
...
{sonarURL}[image:{sonarBadgeURL}&metric=alert_status[Quality Gate Status]]
{sonarURL}[image:{sonarBadgeURL}&metric=security_rating[Security Rating]]
{sonarSummaryURL}[image:{sonarBadgeURL}&metric=coverage[Coverage]]
{sonarURL}[image:{sonarBadgeURL}&metric=ncloc[Lines of Code]]
{sonarSummaryURL}[image:{sonarBadgeURL}&metric=code_smells[Code Smells]]

Give it a try!
https://github.com/th-schwarz/birthday-calendar-service/releases[image:https://img.shields.io/github/v/release/th-schwarz/birthday-calendar-service?include_prereleases[GitHub release (latest by date including pre-releases)]]
image:https://img.shields.io/github/downloads/th-schwarz/birthday-calendar-service/total[GitHub Downloads (all assets, all releases)]
...
│   ├── bcs.jar
│   ├── logback.xml    (logback configuration, optional)
----
...
*logback.xml* is an optional Logback configuration file. If omitted, a console appender is used.  +
...
dav:
  user: dev
  password: strong
  base-url: https://dav.my-domain.org
  cal-path: /SOGo/dav/dav-user/Calendar/46-12345678-5-87654321/
  card-paths:
    - /SOGo/dav/dav-user/Contacts/personal/
----
...
- `dav.base-url`: The base URL of the DAV server (`schema://host(:port)`). All paths below are relative to this server.
- `dav.card-paths`: List of address book paths from which birthdays are read.
- `dav.cal-path`: The path of the birthday calendar which must be created. _To be on the safe side, it's a good idea to use an extra birthday calendar! But it is not mandatory since https://github.com/th-schwarz/birthday-calendar-service/issues/2[A remote event should only be deleted if its categories contain the defined birthday category.]_ was implemented.
...
docker run --rm \
  ...
  -v /path/to/logback.xml:/app/logback.xml:ro `# optional` \
  ghcr.io/th-schwarz/birthday-calendar-service:[tagged-version]
----

Replace `/path/to/bcs.yml` with the absolute path to your configuration file on the host system.
The `-v /path/to/logback.xml` volume mount is optional; omit it to use  a simple console logger.
...
services:
  bcs:
    ...
    environment:
      - LOGGING_CONFIG=/app/logback.xml    # only required when using logback.xml
