...
- `dav.delay-in-minutes`: This property determines the delay interval (in minutes) for scheduled tasks. It's useful in attempting to stagger or retry tasks at specified intervals. Default is _1_.
- `dav.max-trails`: Used to configure the maximum number of retries for operations like synchronization, ensuring stability even in case of intermittent failures. Default is _5_.
